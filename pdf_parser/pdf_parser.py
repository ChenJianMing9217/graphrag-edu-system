...
RE_SCORE_PERCENTILE = re.compile(r"百分位[：:\s]*([<＜]?\s*[\d\.]+)")
RE_SCORE_DQ = re.compile(r"發展商數[：:\s]*([\d\.]+)")
RE_SCORE_SS = re.compile(r"標準分數[：:\s]*([\d\.]+)")
...
class PDFProcessor:
    ...
    def extract_tables_structured(self, page, doc_id: str, page_no: int) -> List[Dict]:
        ...
        for t in tables:
            ...
            is_score = all(any(kw in h for kw in ["原始分數", "標準分數", "百分等級", "發展年齡", "IQ"]) for h in header if h)
            tid = hashlib.sha1(" | ".join(header).encode("utf-8")).hexdigest()[:16]
            for idx, r in enumerate(t[1:]):
...
def parse_scores(lines: List[str]) -> Dict:
    raw = " ".join(lines)
    res = {"percentile": None, "development_quotient": None, "standard_score": None, "current_ability": {"from": None, "to": None}, "raw_text": raw}
    pm = RE_SCORE_PERCENTILE.search(raw)
    if pm:
        try: res["percentile"] = float(pm.group(1).replace("＜", "").replace("<", "").strip()); res["percentile_qualifier"] = "<" if any(c in pm.group(1) for c in "<＜") else None
        except ValueError: pass
    dm = RE_SCORE_DQ.search(raw)
    if dm:
        try: res["development_quotient"] = float(dm.group(1))
        except ValueError: pass
    sm = RE_SCORE_SS.search(raw)
    if sm:
        try: res["standard_score"] = float(sm.group(1))
        except ValueError: pass
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for tbl in merged:
            ...
            for gid, rs in groups.items():
                ...
                if score_buf: p_scores["subtests"], score_buf = score_buf, []
                ...
                if p_scores["raw_text"]: u["scores"] = p_scores
                for k in ["assessment_tools", "observations", "training_directions", "recommendations"]:
                    if p_data.get(k): u[k] = p_data[k]
                units.append(u)
        if score_buf and units: units[-1]["scores"]["subtests"] = units[-1].get("scores", {}).get("subtests", []) + score_buf
        ...
        for u in units:
            ...
            for k, v in u.items():
                if k not in ["category", "item", "normalized_category", "page_start", "page_end", "source_text", "rows"] and v: pdiv[k] = v
